...
const getAllDenuncias = async (req, res, next) => {
    try {
        ...
        denuncias.rows.forEach(denuncia => {
            denuncia.links = [
                { rel: "self", href: `/denuncias/${denuncia.IdDenuncia}`, method: "GET" },
                { rel: "delete", href: `/denuncias/${denuncia.IdDenuncia}`, method: "DELETE" },
                { rel: "modify", href: `/denuncias/${denuncia.IdDenuncia}`, method: "PUT" }
            ];
        });
        ...
        return res.status(200).json({
            ...
            links: [
                ...
                ...(page > 1 ? [{ rel: "pagina-anterior", href: `/denuncias?limit=${limit}&page=${page - 1}`, method: "GET" }] : []),
                ...(denuncias.count > page * limit ? [{ rel: "proxima-pagina", href: `/denuncias?limit=${limit}&page=${+page + 1}`, method: "GET" }] : [])
            ]
        });
    } catch (err) {
        next(err);
...
const createDenuncia = async (req, res, next) => {
    try {
        ...
        const anuncio = await db.Denuncia.findByPk(IdAnuncio);
        if (!anuncio) {
            throw new ErrorHandler(404, 'Denúncia não encontrado');
        }
